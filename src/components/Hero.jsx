import { Heart, ShieldCheck, Leaf, IndianRupee } from "lucide-react";
  ...
  return (
    <section className="relative pt-28 pb-20 overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute -top-32 -right-24 w-[50rem] h-[50rem] bg-amber-300/30 rounded-full blur-3xl" />
        <div className="absolute -bottom-32 -left-24 w-[50rem] h-[50rem] bg-emerald-300/30 rounded-full blur-3xl" />
      </div>

      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-10 items-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <p className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-100 text-orange-800 text-xs font-medium">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <button onClick={onDonateClick} className="inline-flex items-center justify-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-xl shadow hover:bg-emerald-700">
          ...
          <div className="mt-6 flex items-center gap-6 text-sm text-gray-700">
            ...
            <div className="flex items-center gap-2"><Leaf className="text-amber-600" size={18}/>80G receipts</div>
          </div>
        </motion.div>
